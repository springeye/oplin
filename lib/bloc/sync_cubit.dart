...
class SyncCubit extends Cubit<SyncState> {
  ...
  final NotebookCubit notebookLogic;
  ...
  _uploadLocal(WebDAVClient client) async {
    await client.ping();

    var notes = await noteLogic.getNotes().last;
    ...
    for (var localNote in notes) {
      try {
        ...
        if (localNote.deleted) {
          ...
          await client.delete(localNote);
          await noteLogic.deleteNote(localNote.uuid, physics: true);
  ...
  _pullRemote(WebDAVClient client) async {
    await client.ping();
